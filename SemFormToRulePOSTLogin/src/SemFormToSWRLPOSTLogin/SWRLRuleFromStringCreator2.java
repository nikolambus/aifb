...
public class SWRLRuleFromStringCreator2 {
    ...
    // Use here the same IRI as your SMW or Surgipedia
  	public IRI ontologyIRI = IRI.create("http://surgipedia.sfb125.de/wiki/Special:URIResolver/");	
	...
	// This property creates a list of OWL classes and properties from given string
	...
	public List<SWRLHelpClass> buildRulePart(String part, IRI ontologyIRI) throws OWLOntologyCreationException, MalformedURLException, IOException {
		...
		for ( Atom a : Atoms ) {
			...
			if (a.atomType.equalsIgnoreCase("BUILT-IN")) {
				...
				//case "variable": we should take the string without '?' and add both variables to the arguments list					
				...
				//case "literal": we should create an OWLLiteral: either double or integer
				else {
			...
			if (a.atomType == "PROPERTY") {
				...
				Scanner scanner = new Scanner(new URL("http://surgipedia.sfb125.de/wiki/Special:ExportRDF/" + a.atomName).openStream(), "UTF-8").useDelimiter("\\A");
