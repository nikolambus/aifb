...
public class OneRuleMapper7 {
	...
	public String base = "http://surgipedia.sfb125.de/wiki/Special:URIResolver/";
	public String surgi = "http://surgipedia.sfb125.de/wiki/Special:URIResolver/";
	public String surgiProp = "http://surgipedia.sfb125.de/wiki/Special:URIResolver/Property-3A";
	public String surgiVar = "http://surgipedia.sfb125.de/wiki/Special:URIResolver/Variable#";
	public String surgiInd = "http://surgipedia.sfb125.de/wiki/Special:URIResolver/Individual#";
	...
	// Our PLAN
	/* 
	 * 1. "createXMLstructureBody" 
	 * 1.1 "findNumberOfBodyTriples"
	 *         - go through the n3 part rule (snippet) and check for each line "is it a triple?"
	 *         - if it is, save its subject, predicate and object into the corresponding lists
	 *         - note that there are a lot of different triple forms: built-in with multiple input channels, 
	 *           shortcuts with only predicate and object (that refer to the same subject as the previous triple), 
	 *           end with '.' / ';' / nothing, etc.
	 *         - count each triple    
	 * 1.2. Depending from the obtained triple number write in a help file a RDF list of the appropriate depth. 
	 *         - thereby each step lower within the list is reflected by increasing of the tab space.
	 *         - thereby each position where a triple should be placed is marked with the string "Triple". 
	 *      We call this help file "transit" file because it only has an empty list structure and these "Triple" markers.     
	 * 
	 * 2. "createXMLTripleFromN3"
	 *      2.1   
	 *         - go through the transit file and check "is there a triple here?"
	 *         - if it is, get the appropriate items from the subject/predicate/object lists (e.g. 7th subject, predicate and object)          
	 *		   - parse subject: it could be something like this "(?a ?b ?c ?d ?e)" or just "?a" or simply "base:bla"
	 *  	   - parse predicate: it's always like this "prefix:qualityname"
	 *         - parse object: it could be "?c" or "bla blo blu" or "34" 
	 *      2.2 
	 *         - based on i-th subject, predicate and object build the i-th SWRL atom.
	 *         - there are following atoms possible: "DatavaluedPropertyAtom", "IndividualPropertyAtom", "ClassAtom", "BuiltinAtom", "MultiBuiltinAtom"
	 *         - the selection of the atom type is based on predicate
	 *         - write the SWRL atom description into the result string
	 *         - add annotation property from this atom to the rule root which would specify "Is_premise_of" or "Is_conclusion_of"
	 *         
	 * (all operations are done for body and for head as well)
	 * 
	 * 3. "addHasTopic"
	 *          - adds annotation property which refers all concepts occurred in a part rule
	 *          
	 * 4. return the swrl version of the current n3 part rule snippet 
	*/
	...
	public String createXMLTripleFromN3(String rulePart, int tripleNumber, String result, String ruleName, String tab, List<String> allPrefixuris) throws MalformedURLException, IOException {
		...
		//the variable "subject" will store the full URI of appropriate concept after prefix resolving (e.g.: "http://surgipedia.sfb125.de/wiki/Special:URIResolver/Variable#o2"
		String subject ="";
		 ...
		 * the variable "objectFull" will store the full URI after possible prefix resolving (e.g.: "http://surgipedia.sfb125.de/wiki/Special:URIResolver/leaflet_sclerosis")
		 */
		String object ="";
		String objectFull = ""; 	
			...
			while (i < subjectWithPrefix.length()) {
				...
				if (sign.equalsIgnoreCase(" "))
					...
					if (sign.equalsIgnoreCase("?")) {
						...
						multiSubjectList.add("rdf:resource=\"" + surgiVar + subjectWithPrefix.substring(i+1, subjectWithPrefix.indexOf(" ", i)) + "\"/>");
					}
					else {
		...
		//is it a variable?
		if (subjectWithPrefix.charAt(0)=='?') {
			subject = surgiVar + subjectWithPrefix.substring(1, subjectWithPrefix.length());
		...
		if (objectWithPrefix.charAt(0)=='?') {
			objectFull = surgiVar + objectWithPrefix.substring(1, objectWithPrefix.length());
	...
	public String addHasTopic(String allRulesRDF, String ruleName, List<String> allPrefixuris) throws IOException {			
		...
		while ((line = br.readLine()) != null) {
			...
			result = result + line + "\n";
			if (Pattern.matches("\\s*<swrl:Imp rdf:about=\"http://surgipedia.sfb125.de/wiki/Special:URIResolver/" + ruleName + "\">\\s*", line)) {
