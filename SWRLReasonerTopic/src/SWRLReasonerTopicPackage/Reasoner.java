...
public class Reasoner {
    ...
    // Use here the same IRI as your SMW or Surgipedia
  	...
  	// these variables will store reserve copies of the phase individual and appropriate class 
  	// important for the case "no new phase was reasoned"
  	public OWLNamedIndividual currentPhaseInd = null;
    public OWLClass currentPhaseCls = null;
  	
    public OWLOntology action(OWLOntology ruleOnto, OWLOntology patOnto, String outputPath, Boolean doReason) throws OWLOntologyCreationException, OWLOntologyStorageException, IOException {
    	
    	//check
    	System.out.println("rule: " + ruleOnto);
    	System.out.println("patient: " + patOnto);
    	System.out.println("outputPath: " + outputPath);
    	System.out.println("doReason: " + doReason);
		...
		// As far as we have not reached the last rule we should just save the rules together and do not perform reasoning
		if (doReason == false) {
			return mergedOnt;
		}
		else {
			//now we perform: reason + save
			...
			//Now save the result ontology in a new file 
    ...
    // saves the current phase Individual and its class
    ...
    //check do we have got a new phase. If yes, remove the old one. If no, utilize the old one.
