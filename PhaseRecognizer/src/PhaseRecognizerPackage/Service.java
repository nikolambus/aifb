...
public class Service {
	...
	public void postStuff(String rdf, @Context final HttpServletResponse servletResponse, @Context final HttpServletRequest servletRequest, @Context final ServletContext context) throws IOException, SAXException, OWLOntologyCreationException, OWLOntologyStorageException {
		...
		/* String rdf is read (via POST from Postman) and has a rdf/xml structure that should satisfy the pattern in ServiceHelper.getSparqlInputPattern()
			...
			- context URI
			from the POST
			c) OP type URI --> corresponding swrl rules files 
			   context URI --> context owl file
		    d) execute all found rule files on the context file 
		*/
		...
		if (soln==null) 
			...
			*/
			 ...
			 * E.g.: http://surgipedia.sfb125.de/wiki/Special:URIResolver/ContextPancreas1 -> 
			 * phaseContextPrefix = http://surgipedia.sfb125.de/wiki/Special:URIResolver/
	        ...
	        // construct a query which would find ABoxes (context) file given ABoxes (context) URI 
	        ...
	        if (soln1 == null) {
				 ...
				 * E.g.: http://surgipedia.sfb125.de/wiki/Special:URIResolver/PancreasResection -> 
				 * OPTypePrefix = http://surgipedia.sfb125.de/wiki/Special:URIResolver/
		        ...
		        if (solutions.isEmpty()) 
		        	...
		        	for (QuerySolution solnRule : solutions) {
						 ...
						 * http://aifb-ls3-vm2.aifb.kit.edu:8080/PhaseRecognizer/files/output/
						 * 
	...
	public String getServiceDescriptionRDFXML(@Context final HttpServletResponse response, @Context final ServletContext context) throws Exception {
		...
		ServiceHelper.printRDFDescriptionFromFile(descriptionsPath + "PhaseRecognizer_description.xml", response, context, "application/rdf+xml");
