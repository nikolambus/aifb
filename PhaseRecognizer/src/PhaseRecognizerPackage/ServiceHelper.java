...
public class ServiceHelper {
	...
	public static String getSparqlInputPattern() {
		String startFrame = "\n" + "SELECT * WHERE { ";
		String inputPattern = "?request		surgiProp:Has_phaseContext		?phaseContextURI." + "\n" + 
	...
	/* Here we define the SPARQL pattern which finds the swrl rule files by given an OP type URI */
	public static String getSparqlPatternRulesForOPType(String prefix, String opType) {
		...
		String sparqlPattern = "PREFIX prefix: <" + prefix + ">" + "\n" + 
				   ...
				   "?swrlRule surgiProp:Has_OPType prefix:" + opType + "." + "\n" + 
				   "?swrlRule  surgiProp:Has_SWRLRuleFile ?swrlRuleFile . }"; 
		return sparqlPattern;
	}
	
	/* Here we define the SPARQL pattern which finds the owl context file by given a context URI */
	public static String getSparqlPatternForPhaseContextFile(String prefix, String name) {
		
		String sparqlPattern = "PREFIX prefix: <" + prefix + ">" + "\n" + 
				   "PREFIX surgiProp: <http://surgipedia.sfb125.de/wiki/Special:URIResolver/Property-3A>" + "\n" +
				   "\n" + "SELECT ?phaseContextFile WHERE { " + "\n" + 
				   "prefix:" + name + " surgiProp:Has_ABoxesFile ?phaseContextFile . }"; 	
		return sparqlPattern;
	}
