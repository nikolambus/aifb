...
public class AnnotationPropertiesImplantator2 {
	...
	public void action(String inputPath, String ruleName, String OPType, String outputPath) throws IOException {
	    ...
	    while ((line = br.readLine()) != null) {
			...
			if (Pattern.matches("\\s*<owl:Class rdf:about=\"" + base + "([a-zA-Z_0-9:,?\"./]+)\"/>", line)) {
				
				//parse the class name from regex ([a-zA-Z_0-9:,?\"./]+)
				Pattern pattern = Pattern.compile("\\s*<owl:Class rdf:about=\"" + base + "([a-zA-Z_0-9:,?\"./]+)\"/>");
			...
			/*
			 * We have encountered the begin of the rule declaration if 
			...
			/* By encountering the begin of the rule declaration do 
			...
			if (Pattern.matches("\\s*</rdf:Description>\\s*", line)) {
				...
				if ((Pattern.matches("\\s*<rdf:Description>\\s*", nextLine)) && (Pattern.matches("        <rdf:type rdf:resource=\"http://www.w3.org/2003/11/swrl#Imp\"/>", nextnextLine))) {
					...
					writer.println("        <Property-3AHas_name rdf:resource=\"" + base + "/"+ruleName+"\"/>");
			...
			*/
			if (Pattern.matches("\\s*<rdf:type rdf:resource=\"" + base + "([a-zA-Z_0-9:,?\"./]+)\"/>", line)) {
				
				//parse the class name from regex ([a-zA-Z_0-9:,?\"./]+)
				Pattern pattern = Pattern.compile("\\s*<rdf:type rdf:resource=\"" + base + "([a-zA-Z_0-9:,?\"./]+)\"/>");
				Matcher mtch = pattern.matcher(line);
				while (mtch.find()) {
			...
			*/
			if (Pattern.matches("\\s*<rdf:type rdf:resource=\"http://www.w3.org/2003/11/swrl#ClassAtom\"/>\\s*", line)) {
				...
				*/
				...
				Pattern patternCls = Pattern.compile("\\s*<swrl:classPredicate rdf:resource=\"" + base + "([a-zA-Z_0-9:,?\"./]+)\"/>");
				...
				// if we are inside of a body and if the class of the current Atom hasn't been moved from the head side (a kind of exception) --> it's a premise atom
